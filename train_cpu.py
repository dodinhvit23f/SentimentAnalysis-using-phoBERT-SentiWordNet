...
model_bert = RobertaForAIViVN.from_pretrained(args.pretrained_path, config=config) #args.pretrained_path  đường dẫn tới pretrained Bert 
#model_bert.cuda() #đưa lên Cuda

#if torch.cuda.device_count(): # Nếu có cuda
...
X_train = convert_lines(train_df, vocab, bpe,args.max_sequence_length) # chuyen X_train thành ma tran, moi dòng là danh sach các id
...
splits = list(StratifiedKFold(n_splits=10, shuffle=True, random_state=123).split(X_train,  y))  # chia lam 10 phan
...
for  fold, (train_idx, val_idx) in enumerate(splits):
    ...
    if fold != args.fold:
        continue
    train_dataset = torch.utils.data.TensorDataset(torch.tensor(X_train[train_idx],dtype=torch.long), torch.tensor(y[train_idx],dtype=torch.long), torch.tensor(senti[train_idx],dtype=torch.long))
     
    valid_dataset = torch.utils.data.TensorDataset(torch.tensor(X_train[val_idx],dtype=torch.long), torch.tensor(y[val_idx],dtype=torch.long), torch.tensor(senti[val_idx],dtype=torch.long))
    ...
    for epoch in tq:
        ...
        for i,(x_batch, y_batch, senti_batch) in pbar:
            model_bert.train()

            y_pred = model_bert(x_batch, senti_batch, attention_mask=(x_batch>0))
...
X_test = convert_lines(test_df, vocab, bpe,args.max_sequence_length) # chuyen X_train thành ma tran, moi dòng là danh sach các id
test_dataset = torch.utils.data.TensorDataset(torch.tensor(X_test,dtype=torch.long), torch.tensor(y,dtype=torch.long), torch.tensor(senti, dtype=torch.long))
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)
'''
#x_batch = torch.utils.data.TensorDataset(torch.tensor(X_test,dtype=torch.long))
#y_batch= torch.utils.data.TensorDataset( torch.tensor(y,dtype=torch.long))
#senti_batch  = torch.utils.data.TensorDataset(torch.tensor(senti,dtype=torch.long))
...
for i,(x_batch, y_batch, senti_batch) in pbar:
    ...
    y_pred = y_pred.squeeze().detach().cpu().numpy()
    test_preds = np.atleast_1d(y_pred) if val_preds is None else np.concatenate([val_preds, np.atleast_1d(y_pred)])
...
score = f1_score(y, test_preds > 0.5)
score_acc = accuracy_score(y, test_preds > 0.5)
f=open("results.txt", "w")
f.write("accuracy_score: "+str(score_acc)+" \t  f1_score: "+str(f1_score))
f.close()
